...
if __name__ == "__main__":
    ...
    trainloader = torch.utils.data.DataLoader(trainset, batch_size=batch_size, shuffle=True, num_workers=10, persistent_workers=True)

    testset = torchvision.datasets.CIFAR10(root='./data', train=False, download=False, transform=transform)
    testloader = torch.utils.data.DataLoader(testset, batch_size=batch_size, shuffle=True, num_workers=10, persistent_workers=True)

    classes = ('plane', 'car', 'bird', 'cat',
               'deer', 'dog', 'frog', 'horse', 'ship', 'truck')

    torch.distributed.init_process_group(backend='nccl', rank =get_rank(), world_size=get_size())
...
#    net = nn.DataParallel(net)
    ...
    # save checkpoint
    ckpt_path = './checkpoints_test/checkpoint-epoch-{}-batch-{}-trial-{}-.ckpt'.format(num_epochs, batch_size,2)
